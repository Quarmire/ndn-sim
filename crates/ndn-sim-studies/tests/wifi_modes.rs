...
use ndn_sim::{DesKernel, Position, SimKernel, WifiMode};
use ndn_sim_studies::{IpNetwork, RadioLinkConfig, ShortestPath, Wifi, WifiOperatingMode};
...
fn ap_mode_relays_station_to_station_through_the_ap() {
    let (ibss_ap_fwd, ap_ap_fwd, ap_recv) =
        DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
            ...
            let ibss = IpNetwork::from_positions_wifi(
                ...
                pos.clone(),
                &wifi,
                &RadioLinkConfig::new(50.0, WifiMode::Managed),
                &ShortestPath,
            );
            let _ = ibss
                .node(1)
    ...
    assert_eq!(
        ibss_ap_fwd, 0,
        "IBSS: A↔B direct — the AP node relays nothing"
    );
    assert!(
        ap_ap_fwd > 0,
        "AP mode: station→station relays through the AP ({ap_ap_fwd})"
    );
    assert!(
        ap_recv >= 4,
        "the AP-relayed flow still delivers ({ap_recv})"
    );
}
...
fn ap_mode_charges_a_handoff_each_time_a_station_roams_back() {
    let (ap_handoffs, ap_overhead, ibss_handoffs) =
        DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
            ...
            (
                ap.handoff_count(),
                ap.association_overhead(),
                ibss.handoff_count(),
            )
        });
    assert_eq!(
        ap_handoffs, 2,
        "initial join + one roam-back = two associations"
    );
    assert!(
...
fn pluggable_propagation_changes_reach() {
    use ndn_sim_studies::LogDistance;
    let (fs, ld) = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        ...
        let net_fs =
            IpNetwork::from_positions_wifi(k.runtime(), pos.clone(), &wifi, &free, &ShortestPath);
        let fs = net_fs
            .node(0)
            .ping(
                net_fs.addr(1),
        ...
        let lossy = RadioLinkConfig::new(4000.0, WifiMode::Monitor).with_propagation(Arc::new(
            LogDistance {
                exponent: 3.5,
                ref_loss_db: 40.0,
                ref_dist_m: 1.0,
    ...
    assert!(
        fs > ld,
        "free-space reaches farther than a lossy log-distance channel ({fs} vs {ld})"
    );
}
