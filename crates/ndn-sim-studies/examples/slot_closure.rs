...
//!
//! `cargo run --example slot_closure --release -p ndn-sim-studies`. Writes CSV + OTLP-in-Data spans.
    ...
    (0..8)
        .filter_map(|r| {
            ...
            if e >= 0.999 {
                ...
                Some(airtime_ms(K as f64 / (1.0 - e), r))
            }
        })
    ...
    for nm in names {
        let fits = match slot {
            Some(s) => {
                if nm.lease > s {
                    continue;
                } // spills the fixed slot → lost (guard band forbids crossing)
                if t + s > budget {
                    break;
                }
            ...
            None => {
                ...
                if t + need > budget {
                    continue;
                } // try to fit later (smaller) names
                t += need;
                true
            }
        };
...
fn main() {
    ...
    let publisher = SpanPublisher::new(
        Name::from_str("/sim/mac/link/lease/traces").unwrap(),
    ...
    println!(
        "PART A — deliver a name mix in a {BUDGET:.0} ms superframe (N={N} names, airtimes from the"
    );
    println!(
        "link-adaptation decision). Delivery split by CLASS (alarm MUST be served) + airtime eff.\n"
    );
    println!(
        "{:<26}{:>16}{:>12}{:>14}",
        "policy", "alarm served", "bulk served", "airtime eff."
    );
    ...
    for s in [8.0f64, 16.0, 32.0, 64.0, 128.0] {
        ...
        for seed in 0..SEEDS {
            ...
            a_del += o.alarm_del as f64;
            a_tot += o.alarm_tot as f64;
            b_del += o.bulk_del as f64;
            eff += o.efficiency;
        }
        let (n, af) = (SEEDS as f64, a_del / a_tot.max(1.0));
        println!(
            "{:<26}{:>13.0}% ({:.1}){:>12.1}{:>13.0}%",
            format!("fixed slot {s:.0} ms"),
            af * 100.0,
            a_del / n,
            b_del / n,
            eff / n * 100.0
        ...
        otlp.span(
            ...
            vclock,
            vec![
                Attr::str("slot_ms", format!("{s:.0}")),
                Attr::int("alarm_served_pct", (af * 100.0) as i64),
                Attr::int("bulk_served", (b_del / n) as i64),
                Attr::int("eff_pct", (eff / n * 100.0) as i64),
            ],
        );
    }
    ...
    println!(
        "{:<30}{:>16}",
        "worst-rate disagreement", "delivered (no desync)"
    );
    for err in [0.0f64, 0.05, 0.15, 0.30] {
        ...
        for seed in 0..SEEDS {
            ...
            for nm in &names {
                ...
                let cons_lease = if rng.f() < err {
                    nm.lease * (0.6 + rng.f() * 0.8)
                } else {
                    nm.lease
                };
        ...
        println!(
            "{:<30}{:>16.1}",
            format!("{:.0}% of names misread", err * 100.0),
            survive
        );
    ...
    println!(
        "A: NO fixed slot wins. Small slots (8-16 ms) serve ~0% of ALARM names — their worst-receiver"
    );
    println!(
        "   leases all exceed the slot, so they spill the boundary and are lost — while gaming the raw"
    );
    println!(
        "   name count with cheap bulk. Large slots (64-128 ms) serve alarms but waste airtime (low"
    );
    println!(
        "   efficiency) and starve bulk throughput. Rate-derived leases serve the alarm class AND run"
    );
    println!(
        "   ~98% efficient — because the lease IS the link decision, sized to each name's true need."
    );
    println!(
        "B: but the lease must be SHARED-computable — divergent worst-rate reads desync the schedule"
    );
