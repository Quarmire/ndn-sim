...
//! and report both sides with the same protocol-neutral [`FlowStats`]. Because the
...
use ndn_sim::app::{AppSpec, FlowStats, TrafficPattern};
use ndn_sim::sim_link::{FaceProfile, LinkConfig};
...
fn run_ndn(spec: &ComparisonSpec) -> (FlowStats, u64) {
    use ndn_sim::scenario::{KernelSpec, NodeSpec, Scenario, ScenarioLink};
    use ndn_sim::{DesKernel, NodeId, SimKernel};
    ...
    ndn_sim::topo::add_routes_toward(&mut scenario, "/bench", spec.dest);
