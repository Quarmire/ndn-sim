...
//!
//! `cargo run --example multi_radio_opt --release -p ndn-sim-studies`. Writes CSV.
...
fn value(demand: &[f64], r_movers: usize, sensed: &[usize], bad: &[usize]) -> f64 {
    ...
    let dest = |c: usize| -> usize {
        if believed_clear.is_empty() {
            c
        } else {
            believed_clear[(c.wrapping_mul(2654435761)) % believed_clear.len()]
        }
    };
    ...
    let mut del = 0.0;
    for (c, &d) in demand.iter().enumerate().take(C) {
        let landing = if bad.contains(&c) { dest(c) } else { c };
        if !bad.contains(&landing) && movers.contains(&landing) {
            del += d; // delivered iff it landed on a truly-clear channel a mover covers
        }
    }
    del
}
...
fn main() {
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/data/spectrum-access");
    ...
    println!(
        "diversity = 1-(1-p)^2 on ALL demand; different-channel coverage = p on top-2 channels' demand."
    );
    println!(
        "{:<10}{:>18}{:>18}",
        "p (margin)", "coverage (spread)", "diversity (combine)"
    );
    ...
    println!(
        "   sense (correct avoidance) — wins at low R; optimizer+coop (consume a neighbour's map) is best"
    );
    println!(
        "   AND makes R=1 viable. Diminishing returns past R=2-3 — matching '4+ radios is uncommon'."
    );
    println!(
        "B: on a MARGINAL link (low p) diversity (combine 2 radios on one channel) beats coverage; on a"
    );
    println!(
        "   GOOD link coverage (spread) wins. The optimizer picks per link margin + demand spread."
    );
