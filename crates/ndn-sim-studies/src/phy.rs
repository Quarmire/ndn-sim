...
//!
//! **Scope note (F7):** the NDN radio face — [`RadioBus`](ndn_sim::RadioBus) in the core crate —
//! computes propagation and SINR through [`ndn_sim::medium`]'s `PropagationModel`, **not** through
//! this module's `RadioEnvironment` / antenna / self-interference composition. `phy` backs the
//! studies' IP-over-radio links ([`RadioLinkConfig`](crate::RadioLinkConfig)) and **LoRa**
//! ([`crate::lora`]) via `PropagationBackend`; the richer multi-radio/antenna model here is a
//! reference not wired to `RadioBus`, so `medium.rs` stays the single source of truth for the
//! simulated NDN radio face.
//!
...
use ndn_sim::medium::free_space_path_loss_db;
...
/// Friis free-space: `Prx = Ptx + Gtx + Grx − FSPL(d, f)`, FSPL from core's
/// [`free_space_path_loss_db`] — the same formula the NDN radio face's `FreeSpacePathLoss` uses.
#[derive(Clone, Copy, Debug, Default)]
pub struct FreeSpace;
impl PropagationBackend for FreeSpace {
    fn rx_power_dbm(&self, ctx: &PathContext) -> f64 {
        ctx.tx_power_dbm + ctx.tx_gain_dbi + ctx.rx_gain_dbi
            - free_space_path_loss_db(ctx.distance_m, ctx.freq_hz)
    }
    fn name(&self) -> &'static str {
        "free-space"
    }
