...
//! tail). `cargo run --example fec_pooling --release -p ndn-sim-studies`. Writes CSV + OTLP-in-Data spans.
...
fn main() {
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/data/link-adapt");
    ...
    let publisher = SpanPublisher::new(
        Name::from_str("/sim/mac/link/pooling/traces").unwrap(),
    ...
    println!(
        "PART A — the ephemeral COUNT drives the FEC discount (phy={phy}, any-of, independent)."
    );
    println!(
        "{:<6}{:>10}{:>10}{:>10}{:>16}{:>16}",
        "n", "eff=phy^n", "parity", "N", "airtime (rel)", "pool-delivery"
    );
    let base_n = {
    ...
    for n in 1..=6usize {
        ...
        writeln!(
            csv,
            "A,discount,{n},{:.4},parity={parity};N={big_n};deliver={:.3}",
            air, deliver
        )
        .ok();
        let start = vclock;
        vclock += 1000;
        otlp.span(
            "mac.pooling.discount",
            SpanKind::Internal,
            start,
            vclock,
            vec![
                Attr::int("n", n as i64),
                Attr::int("parity", parity as i64),
                Attr::int("airtime_pct", (air * 100.0) as i64),
                Attr::int("deliver_pct", (deliver * 100.0) as i64),
            ],
        );
    }
    println!(
        "  ⇒ redundancy + airtime SHRINK as the ephemeral count grows (46→32, ~30% less), pool-"
    );
    println!(
        "    delivery holds. This is the honest contribution of the §2 nonce: the count, not identity."
    );
    ...
    println!(
        "{:<6}{:>18}{:>20}{:>18}",
        "n", "any-of pool-deliver", "all-of w/ discount", "all-of sized right"
    );
    // correct all-of parity: size so ONE receiver decodes ≥99% at raw phy (no n-discount).
    let mut n_allof = K;
    while binom_ge(n_allof, K, 1.0 - phy) < 0.99 {
        n_allof += 1;
    }
    for n in 1..=6usize {
        let eff = phy.powi(n as i32);
        ...
        println!(
            ...
            n,
            anyof * 100.0,
            allof_discounted * 100.0,
            allof_right * 100.0
        );
        writeln!(
            csv,
            "B,semantics,{n},{:.4},anyof={:.3};allof_right={:.3}",
            allof_discounted, anyof, allof_right
        )
        .ok();
    }
    println!(
        "  ⇒ any-of holds; the SAME discount on an all-of name collapses toward 0 (parity shrinks when"
    );
    println!(
        "    all-of needs it to GROW). all-of sized right (N={n_allof}, no n-discount) holds ~99%^n."
    );
    println!(
        "    The name's reliability semantics MUST gate the pooling discount — it is not free."
    );
    ...
    println!(
        "it. n=4, parity sized for phy^4 (assumes independence); actual loss has a common component c."
    );
    let n = 4usize;
    ...
    println!(
        "{:<28}{:>18}{:>16}",
        "loss correlation (common c)", "pool per-frame rx", "pool-delivery"
    );
    for c in [0.0f64, 0.05, 0.15, 0.30] {
        ...
        vclock += 1000;
        otlp.span(
            "mac.pooling.correlation",
            SpanKind::Internal,
            start,
            vclock,
            vec![
                Attr::str("c", format!("{c:.2}")),
                Attr::int("p_pool_pct", (p_pool * 100.0) as i64),
                Attr::int("deliver_pct", (deliver * 100.0) as i64),
            ],
        );
    }
    println!(
        "  ⇒ as loss correlates (all receivers behind one interferer), all-miss rises from phy^n toward"
    );
    println!(
        "    phy, the independence-sized parity under-provisions, and pool-delivery collapses. The"
    );
    println!(
        "    discount needs the losses to actually be independent — contention (correlated) voids it."
    );
    ...
    println!(
        "HEADLINE: the ephemeral count is a REAL FEC lever (Part A: ~30% airtime off at n≥3) but a"
    );
    println!(
        "DOUBLY-GATED one — valid only for any-of names (Part B) with independent loss (Part C). Blind"
    );
    println!(
        "application under-provisions an alarm (all-of) name or a contended (correlated) channel. So"
    );
    println!(
        "the pooling discount must read the name's reliability semantics AND a correlation estimate,"
    );
    println!(
        "not just the count. The ephemeral nonce supplies the count honestly; the gates are the design."
    );
