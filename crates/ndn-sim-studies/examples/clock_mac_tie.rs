...
//!
//! We sweep guard × residual (labelled by the clock discipline that produces it) and report
//! delivery + goodput + access latency, all from the real RadioBus collision model. The payoff: which clock
//! tier unlocks which guard, and what that guard costs in latency/goodput.
//!
//! `cargo run --example clock_mac_tie --release -p ndn-sim-studies`. Writes CSV + a JSON line.
...
fn main() {
    use std::io::Write;
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/data/clock-phase");
