...
use ndn_sim::{
    LinkConfig, MetricsLog, MetricsSample, SimSpanEmitter, SimKernel, Simulation, VirtualKernel,
};
...
async fn metrics_scenario(kernel: Arc<dyn SimKernel>) -> Vec<MetricsSample> {
    ...
    sim.link(
        a,
        b,
        LinkConfig { delay: Duration::from_millis(10), ..LinkConfig::default() },
    );
...
fn gauge_emitter_series_is_virtual_timed_reflects_traffic_and_replays() {
    ...
    assert_eq!(first, second, "the metric series replays identically under virtual time");

    // 2 nodes × 5 ticks.
    assert_eq!(first.len(), 10, "5 ticks for each of 2 nodes, got {}", first.len());
    ...
    for w in node0.windows(2) {
        assert_eq!(w[1] - w[0], 1_000_000_000, "samples 1 s of virtual time apart");
    }
}
...
fn span_timestamps_track_virtual_time_and_replay() {
    let capture = |kernel: Arc<dyn SimKernel>| async move {
        ...
        let publisher =
            SpanPublisher::new("/sim/obs".parse::<Name>().unwrap(), SpanRetention::default());
    ...
    assert_eq!(ts_a, now_a, "span timestamp == virtual clock, not wall-clock");
