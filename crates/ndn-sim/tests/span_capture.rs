...
use ndn_sim::{
    CapturedSpan, ControlPlane, LinkConfig, SimKernel, SimMcp, Simulation, SpanLog, VirtualKernel,
...
async fn exchange(k: Arc<dyn SimKernel>) -> ndn_sim::RunningSimulation {
    ...
    tokio::spawn(async move {
        let _ = producer
            .serve(|i, r| async move {
                let _ = r
                    .respond((*i.name).clone(), bytes::Bytes::from_static(b"x"))
                    .await;
            })
            .await;
    });
    ...
    let builder =
        InterestBuilder::new("/app/0".parse::<Name>().unwrap()).lifetime(Duration::from_secs(20));
...
fn engine_spans_captured_on_the_virtual_clock() {
    ...
    assert!(
        !entries.is_empty(),
        "engine emitted spans/events into the log"
    );
    // Timestamps are virtual (the engine's clock), not wall-clock.
    assert!(
        entries
            .iter()
            .all(|e| e.virtual_time_ns >= 1_700_000_000_000_000_000),
    ...
    assert!(
        entries
            .iter()
            .any(|e| e.target.starts_with("fwd") || e.target.starts_with("engine")),
...
fn why_did_returns_the_engine_causal_trace() {
    let trace_len = VirtualKernel::new().run(|k| async move {
        ...
        let v = mcp
            .call_tool("why_did", &serde_json::json!({ "limit": 50 }))
            .await
            .unwrap();
    ...
    assert!(
        trace_len > 0,
        "why_did surfaced the engine causal trace, not just a flat event log"
    );
}
