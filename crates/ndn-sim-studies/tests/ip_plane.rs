...
use ndn_sim_studies::{IpNode, Ipv4, ip_link};
...
fn ip_line_forwards_and_pings_end_to_end() {
    ...
    assert!(
        stats.received >= 9,
        "pings echoed end-to-end over IP forwarding: {stats:?}"
    );
    // 2 hops each way over a 1 ms link → RTT ≈ 4 ms.
    let rtt = stats.mean_rtt_ms();
    assert!(
        (2.0..20.0).contains(&rtt),
        "IP round-trip measured, got {rtt} ms"
    );
    // The relay forwarded every request AND every reply (≈ 2 per successful ping).
    assert!(
        relay_forwarded >= 18,
        "relay forwarded requests + replies: {relay_forwarded}"
    );
}
...
fn ip_no_route_drops_and_is_deterministic() {
    ...
    assert!(
        dropped >= 5,
        "the relay dropped the unroutable packets: {dropped}"
    );
    assert_eq!(
        run(),
        (received, lost, dropped),
        "IP forwarding replays identically on DES"
    );
}
...
fn ip_network_auto_routes_a_line() {
    ...
    assert!(
        (5.0..25.0).contains(&rtt_ms),
        "RTT ≈ 8 ms over 4 hops, got {rtt_ms}"
    );
}
...
fn ip_run_flow_over_scenario_graph() {
    use ndn_sim::{TrafficPattern, topo};
    use ndn_sim_studies::IpNetwork;
    let recv = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        ...
        let stats = net
            ...
            .run_flow(
                net.addr(3),
                TrafficPattern::Cbr { interval_ms: 5 },
                10,
                16,
                Duration::from_secs(1),
            )
    ...
    assert!(
        recv >= 9,
        "run_flow over a scenario-built IP network delivered: {recv}"
    );
}
...
fn distance_vector_routes_over_the_shortcut() {
    use ndn_sim_studies::{DistanceVector, IpNetwork};
    ...
    assert!(
        rtt < 3.5,
        "DV took the 1-hop diagonal (≈2 ms), got {rtt} ms"
    );
}
...
fn geographic_routing_delivers_by_position() {
    use ndn_sim::Position;
    use ndn_sim_studies::{GreedyGeographic, IpNetwork};
    let recv = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        let rt = k.runtime();
        let prof = FaceProfile::internal().with_link(LinkConfig::lan());
        let positions = vec![
    ...
    assert!(
        recv >= 5,
        "GPSR greedy delivered along the line by position: {recv}"
    );
}
...
fn reactive_routing_delivers_and_costs_less_for_one_flow() {
    use ndn_sim_studies::{Aodv, IpNetwork, RoutingAlgorithm, ShortestPath, TopologyView};
    let (recv, aodv_overhead, sp_overhead) =
        DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
            let rt = k.runtime();
            let prof = FaceProfile::internal().with_link(LinkConfig::lan());
            ...
            (
                stats.received,
                Aodv.control_overhead(&view, one_flow),
                ShortestPath.control_overhead(&view, one_flow),
            )
        });
    assert!(
        recv >= 7,
        "AODV discovered the route and delivered end-to-end: {recv}"
    );
    assert!(
...
fn ip_metrics_and_handoffs_reach_the_otlp_exporter() {
    use ndn_sim::{OtlpExporter, Position, WifiMode};
    use ndn_sim_studies::{RadioLinkConfig, ShortestPath, Wifi, WifiOperatingMode};
    let (ip_payload, gauges_payload) = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        ...
        let ip = ndn_sim_studies::ip_metrics_payload(&exporter, &net.metrics_snapshot());
    ...
    let metrics = ip["resourceMetrics"][0]["scopeMetrics"][0]["metrics"]
        .as_array()
        .unwrap();
    let fwd = metrics
        .iter()
        ...
        .unwrap();
    let ap_fwd = fwd["gauge"]["dataPoints"]
        .as_array()
        .unwrap()
        .iter()
        .find(|p| p["attributes"][0]["value"]["intValue"] == "0")
        .unwrap();
    assert_ne!(
        ap_fwd["asDouble"], 0,
        "the AP node's forwarded count is exported"
    );

    // The roaming handoffs surface as a fabric gauge.
    let g: serde_json::Value = serde_json::from_str(&gauges_payload).unwrap();
    let gm = g["resourceMetrics"][0]["scopeMetrics"][0]["metrics"]
        .as_array()
        .unwrap();
    let ho = gm
        .iter()
    ...
    assert!(
        hv >= 2.0,
        "handoffs (initial join + roam-back) exported as a gauge: {hv}"
    );
}
...
fn ip_runs_over_a_long_range_lora_link() {
    use ndn_sim::Position;
    use ndn_sim_studies::{IpNetwork, LoraLinkConfig, ShortestPath, SpreadingFactor};
    let (recv, rtt_ms) = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        let rt = k.runtime();
    ...
    assert!(
        rtt_ms > 500.0,
        "LoRa's long airtime dominates the round-trip: {rtt_ms} ms"
    );
}
...
fn gpsr_delivers_across_a_void_that_greedy_drops() {
    use ndn_sim::Position;
    use ndn_sim_studies::{Gpsr, GreedyGeographic, IpNetwork};
    let (greedy_recv, gpsr_recv) = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        ...
        let g = IpNetwork::from_links_with(
            Arc::clone(&rt),
            5,
            &links,
    ...
    assert!(
        gpsr_recv >= 3,
        "GPSR perimeter recovery delivers around the void: {gpsr_recv}"
    );
}
...
fn ip_reroutes_around_a_cut_link() {
    use ndn_sim_studies::{IpNetwork, ShortestPath};
    ...
    assert!(
        after >= 2,
        "re-routing around the cut restored delivery: {after}"
    );
}
