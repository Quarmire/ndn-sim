...
async fn governor_runs_a_fabric_on_a_logical_clock_at_real_pace() {
    ...
    assert!(
        logical < real_now,
        "logical clock ({logical}) is scenario-relative, not the wall clock"
    );
    assert!(
        logical >= 1_700_000_000_000_000_000,
        "logical epoch base present"
    );

    // Both engines share the one kernel clock.
    let logical_b = fabric.engine_of(b).unwrap().runtime().unix_nanos();
    assert!(
        logical_b.abs_diff(logical) < 1_000_000_000,
    ...
    let builder =
        InterestBuilder::new("/app/ping".parse::<Name>().unwrap()).lifetime(Duration::from_secs(5));
    let data = consumer
        .fetch_with(builder)
        .await
        .expect("exchange under the governor");
    assert_eq!(
