...
//!
//! `cargo run --example multi_radio --release -p ndn-sim-studies`. Writes CSV.
    ...
    for nm in 0..NAMES {
        ...
        let ch = if bad.contains(&base) && sensed_set.contains(&base) {
            ...
            if believed_clear.is_empty() {
                ...
                believed_clear[(hh(nm as u64) as usize) % believed_clear.len()]
            }
        } else {
        ...
        if !bad.contains(&ch) {
            del += 1;
        } // delivered iff the channel we ended on is truly clear
    }
...
fn main() {
    ...
    for s in [1usize, 2, 4, 6, 8] {
        ...
        d /= 40.0;
        let label = if s == 1 {
            "1  (single radio — blind)"
        } else if s == C {
            "8  (full pool coverage)"
        } else {
            ""
        };
    ...
    println!(
        "   coverage (a sensor radio / the pool) makes avoidance WORK. Multi-radio fills the map §9.2 needs."
    );
    println!(
        "B: one 16ms-retune radio can hold ONE channel → covers only the top-demand slice; more radios"
    );
    println!(
        "   cover more of the node's interests simultaneously — the retune tax's answer is more radios."
    );
