...
//!
//! Run: `cargo run -p ndn-sim-studies --example channel_diversity`
...
use ndn_sim::{
    AdjacentLeakChannel, CarrierSenseInterference, FreeSpacePathLoss, Position, Simulation,
};
...
fn cost(c: i32, load: &[u32], jam: Option<u8>) -> f64 {
    ...
    if let Some(j) = jam {
        ...
        if d == 0 {
            ...
            k += 20.0
        } // jammer collides on its channel + adjacent
    }
    k
}
...
fn assign(strat: Strat, prefixes: &[String], jam: Option<u8>) -> Vec<u8> {
    ...
    match strat {
        ...
        Strat::Separated => {
            let classes = (C as u64).div_ceil(2); // even channels 0,2,4,...
            prefixes
                .iter()
                .map(|p| ((fnv(p) % classes) * 2) as u8)
                .collect()
        }
        Strat::SenseAvoid => {
        ...
        Strat::Oracle => {
            ...
            if let Some(j) = jam {
                usable.retain(|&c| (c as i32 - j as i32).abs() > 1);
            }
            if usable.is_empty() {
                usable.push(if jam == Some(0) { 4 } else { 0 });
            }
...
async fn run(strat: Strat, jam: Option<u8>) -> f64 {
    ...
    for (i, (prod, cons)) in pairs.iter().enumerate() {
        ...
        consumers.push((fabric.engine_of(*cons).unwrap(), prefix, counted));
    }
            ...
            workers.push((
                ...
                tokio::spawn(async move {
                    while Instant::now() < deadline {
                        ...
                        if fetch(&eng, &format!("{prefix}/seg{s}")).await {
    ...
    for (ok, counted, h) in workers {
        let _ = h.await;
        if counted && seen.insert(Arc::as_ptr(&ok)) {
...
async fn main() {
    println!(
        "Channel-diversity guarantee — 3 fixes vs naive baseline + oracle ceiling, real engine + leak\n"
    );
    let strats = [
    ...
    for (name, s) in strats {
        println!(
            "    {:<18} sym {:?}   jam@{JAM_CH} {:?}",
            name,
            assign(s, &prefixes, None),
            assign(s, &prefixes, Some(JAM_CH))
        );
    }
    ...
    println!(
        "STATELESS separated-classes recovers most of it for free. SOFT-STATE (recomputable table,"
    );
    println!(
        "doctrine §7) is best; SENSE-AVOID is nearly as good and fully local. The surprise: the thesis's"
    );
    println!(
        "rigid LINK GUARANTEE (never adjacent, avoid the interferer) is NOT the ceiling — it sacrifices"
    );
    println!(
        "channels, so under scarcity it is BEATEN by the stateless/soft approaches that tolerate a little"
    );
    println!(
        "leak or balance adaptively. So the one thing DCNLA had that named-data 'gave up' does not need"
    );
    println!(
        "its rigid form back: a stateless separated-class hash (default) + soft-state/sense-avoid when"
    );
    println!(
        "contended dominates it — and none of them reintroduces host identity. (Real engine; ±~12 noise.)"
    );

    eprintln!(
        "{{\"pipes\":{PIPES},\"C\":{C},\"jam\":{JAM_CH},\"rows\":[{}]}}",
        rows.join(",")
    );
}
