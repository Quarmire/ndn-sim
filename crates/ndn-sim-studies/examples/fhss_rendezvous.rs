...
//!
//! Run: `cargo run -p ndn-sim-studies --example fhss_rendezvous`
...
use ndn_sim::{
    AdjacentLeakChannel, CarrierSenseInterference, FreeSpacePathLoss, NodeId, Position, Simulation,
};
...
async fn fetch(engine: &ndn_engine::ForwarderEngine, name: &str) -> bool {
    ...
    let b =
        InterestBuilder::new(name.parse::<Name>().unwrap()).lifetime(Duration::from_millis(300));
    matches!(
        tokio::time::timeout(Duration::from_millis(300), consumer.fetch_with(b)).await,
        Ok(Ok(_))
    )
}
...
async fn run(hopping: bool, jam: bool) -> (Vec<f64>, f64) {
    ...
    let per_flow: Vec<(Arc<AtomicU64>, Arc<AtomicU64>)> = (0..FLOWS)
    ...
    let ratios: Vec<f64> = per_flow
        ...
        .map(|(a, o)| {
            ...
            if a == 0 {
                ...
                o.load(Ordering::Relaxed) as f64 / a as f64
            }
        })
        .collect();
    let agg = per_flow
        .iter()
        .map(|(_, o)| o.load(Ordering::Relaxed))
        .sum::<u64>() as f64
        / RUN.as_secs_f64();
...
async fn main() {
    ...
    println!(
        "hop set = {CLASSES:?} (separated classes, #66); {FLOWS} flows; jammer static on ch{JAM_CH}\n"
    );
    ...
    let cfgs = [
        ("static  · no jam", false, false),
        ("static  · jam@2", false, true),
        ("hopping · no jam", true, false),
        ("hopping · jam@2", true, true),
    ];
    ...
    println!(
        "shared epoch clock keep every flow's endpoints on the same channel, no coordinator. WITH a"
    );
    println!(
        "persistent single-channel jammer, the STATIC hash strands whatever flow hashed onto that"
    );
    println!(
        "channel (worst-flow → ~0), while HOPPING spreads every flow across the class set so the"
    );
    println!(
        "jammer only costs each flow ~1/C of its airtime — no flow is ever killed. FHSS turns a fatal"
    );
    println!(
        "collision into a graceful, fair tax, and the schedule is still just H(name) — no host identity."
    );

    eprintln!(
        "{{\"flows\":{FLOWS},\"classes\":{CLASSES:?},\"jam\":{JAM_CH},\"rows\":[{}]}}",
        rows.join(",")
    );
}
