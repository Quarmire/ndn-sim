...
//!
//! `cargo run --example clock_phase_gap --release -p ndn-sim-studies`. Writes CSV + a JSON line.
    ...
    for r in 0..ROUNDS {
        ...
        for (i, &(seq, wall, unc, cap)) in published.iter().enumerate() {
            for (j, node) in nodes.iter_mut().enumerate() {
                ...
                node.tk.ingest_beacon(i as u64, &beacon);
            }
        }
...
fn main() {
    ...
    println!("{:<8} residual (µs) at hop 1,2,…", "arm");
    let tx = 50_000;
    for (name, arm) in [
        ("air", Arm::Air),
        ("build+cal", Arm::BuildCal),
    ...
    println!("{:<10} residual (µs) at hop 1..8", "arm");
    for (name, arm) in [
        ("air", Arm::Air),
        ("shared", Arm::Shared),
        ("build+cal", Arm::BuildCal),
