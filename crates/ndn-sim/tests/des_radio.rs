...
fn radio_exchange() -> Vec<u8> {
    DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        ...
        fabric.route_over_radio(a, &"/svc".parse::<Name>().unwrap()).unwrap();
        let mut consumer = fabric.engine_of(a).unwrap().app_consumer(CancellationToken::new());
        let builder =
            InterestBuilder::new("/svc/0".parse::<Name>().unwrap()).lifetime(Duration::from_secs(20));
        let data = consumer.fetch_with(builder).await.expect("fetch over radio on DES");
...
fn radio_fabric_exchanges_over_the_des_event_queue() {
    assert_eq!(radio_exchange(), b"air", "Interest+Data crossed the radio medium on the event queue");
}

#[test]
fn radio_fabric_replays_deterministically_on_des() {
    assert_eq!(radio_exchange(), radio_exchange(), "radio fabric replays identically on the event queue");
}
