...
//!
//! ```sh
//! NDR_PREFIX_DEPTH=2 NDR_STRATEGY=soft-prefix-reach-defer cargo run --example ndr_multiproducer -p ndn-sim-studies
//! NDR_PREFIX_DEPTH=2 NDR_STRATEGY=soft-prefix-reach-bloom NDR_BLOOM_CELLS=256 cargo run --example ndr_multiproducer -p ndn-sim-studies
//! NDR_PREFIX_DEPTH=2 NDR_STRATEGY=soft-prefix-reach-bloom NDR_BLOOM_CELLS=8   cargo run --example ndr_multiproducer -p ndn-sim-studies
//! ```
use std::sync::Arc;
...
fn main() {
    ...
    println!(
        "(memory footprint: scalar map = O(K) entries; counting Bloom = fixed {cells} cells regardless of K)"
    );
}
