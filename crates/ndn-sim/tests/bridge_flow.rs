...
async fn foreign_flow_experiences_configured_loss_and_delay() {
    ...
    // ── delay probe: lossless, 120 ms — one datagram, timed clean ────────────
    {
        ...
        let mut buf = [0u8; 64];
        tokio::time::timeout(Duration::from_secs(2), b.recv_from(&mut buf))
            .await
        ...
        assert!(
            ...
            "the 120 ms link delay is applied, not passthrough: {d:?}"
        );
    }
    ...
    const SENT: usize = 60;
    for i in 0..SENT {
        a.send_to(format!("rc-frame-{i}").as_bytes(), relay)
            .await
            .unwrap();
    ...
    assert!(
        arrived > 0 && arrived < SENT,
        "loss applied, not lossless and not total: {arrived}/{SENT}"
    );
}
...
async fn foreign_flow_is_bidirectional() {
    ...
    let cancel = CancellationToken::new();
    let relay = fabric
        .bridge_udp_flow(addr_a, addr_b, link, cancel.clone())
        .await
        .unwrap();
    ...
    let (n, _) = tokio::time::timeout(win, peer_b.recv_from(&mut buf))
        .await
    ...
    let (n, _) = tokio::time::timeout(win, peer_a.recv_from(&mut buf))
        .await
