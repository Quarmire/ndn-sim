//! A **statistical 802.11 MAC** on top of core's [`LinkModel`] PHY and airtime model
//! ([`ndn_sim::wifi`]) — what the IP plane's Wi-Fi links run over. The PHY says *whether a frame at
//! a rate is decoded*; this layer models what the MAC does with that:
//!
//! - **Monitor / injection** ([`Wifi::monitor_tx`]): one broadcast, no ACK, no retry.
//! - **Managed unicast** ([`Wifi::managed_unicast_tx`]): EDCA CSMA-CA + ACK + retransmission up to
//!   a retry limit, with **rate adaptation** (a Minstrel-HT-style controller picking the MCS that
//!   maximises expected throughput from per-link success history).
//! - **Operating modes** ([`WifiOperatingMode`]): IBSS / AP / mesh connectivity + association cost.
//!
//! *Logically* faithful, not calibrated: real Minstrel-HT, EDCA, aggregation (A-MPDU) and block-ACK
//! are abstracted, but the relationships — retries raise delivery, higher MCS is cheaper airtime but
//! needs more SNR, rate control tracks the channel — are modelled. Airtime comes from core's
//! [`ndn_sim::wifi`] so the IP plane and the NDN radio face charge identical on-air time.

use std::collections::HashMap;
use std::time::Duration;

use ndn_sim::link_model::{LinkModel, MAX_RELIABLE_MCS, mcs_phy_rate_bps};
use ndn_sim::wifi::{AccessCategory, WifiMode, broadcast_airtime, managed_unicast_attempt_airtime};
use rand::Rng;
use rand::rngs::StdRng;

/// Managed multicast/broadcast goes out at a **basic (legacy) rate** — MCS 0 here — with no ACK and
/// no rate adaptation, per 802.11's group-addressed-frame rules.
const BASIC_RATE_MCS: u8 = 0;

/// The 802.11 **operating mode** — how nodes organise, distinct from [`WifiMode`] (which is the
/// PHY/MAC discipline). This shapes the connectivity graph and the association cost. A
/// [`RadioLinkConfig`](crate::RadioLinkConfig) carries one of each — e.g. `Managed` + `Ap` = normal
