...
async fn model_builds_a_line_topology_via_mcp_tools() {
    ...
    assert!(caps["commands"].as_array().unwrap().iter().any(|c| c == "connect"));
    ...
    assert_eq!(n1["id"], 1);
    let n2 = tool(&mcp, 3, "spawn_node", serde_json::json!({ "label": "leaf" })).await;
    assert_eq!(n2["id"], 2);

    tool(&mcp, 4, "connect", serde_json::json!({ "a": 0, "b": 1, "delay_ms": 5 })).await;
    tool(&mcp, 5, "connect", serde_json::json!({ "a": 1, "b": 2, "delay_ms": 5 })).await;
    tool(&mcp, 6, "route", serde_json::json!({ "node": 0, "prefix": "/leaf", "nexthop": 1 })).await;
    ...
    assert!(topo["links"].as_array().unwrap().len() >= 4, "two bidirectional links");
    ...
    assert!(state["metrics"]["pit_depth"].is_number(), "metrics present: {state}");
    assert!(!state["links"].as_array().unwrap().is_empty(), "node 1 is linked");
