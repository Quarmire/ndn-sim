...
use crate::cosim::{ChannelSource, NodeState};
...
/// Connect to `cfg.endpoint`, spawn a blocking reader thread that decodes vehicle positions and
/// pushes [`NodeState`]s into the returned [`ChannelSource`]. Feed the source to
...
pub fn mavlink_source(cfg: MavlinkConfig) -> Result<(ChannelSource, MavlinkReader)> {
    ...
    let conn = mavlink::connect::<MavMessage>(&cfg.endpoint)
        .with_context(|| format!("connect MAVLink endpoint {:?}", cfg.endpoint))?;
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while !stop_thread.load(Ordering::Relaxed) {
                ...
                // Adopt the first global fix as the ENU origin if none was configured.
                if origin.is_none() {
    ...
    Ok((source, MavlinkReader { stop, handle: Some(handle) }))
}
