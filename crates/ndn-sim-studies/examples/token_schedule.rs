...
//!
//! Run: `cargo run -p ndn-sim-studies --example token_schedule`
...
fn p_col(k: usize) -> f64 {
    if k <= 1 {
        0.0
    } else {
        1.0 - (1.0 - GUARD).powi((k - 1) as i32)
    }
}
...
fn run(sched: Sched, active: usize, seed: u64) -> Out {
    ...
    let mut last = [u64::MAX; N]; // last slot each active name transmitted
    ...
    for t in 0..SLOTS {
        let served: Option<usize> = match sched {
            ...
            Sched::Demand => {
                ...
                if o < active {
                    ...
                    coll += 1;
                    None // owner idle → reclaim via CCLF among the active names
                } else {
                    Some(winner(&mut rng))
                }
            }
        ...
        if let Some(name) = served {
            delivered += 1;
            if last[name] != u64::MAX {
                gaps.push(t - last[name]);
            }
            last[name] = t;
        }
    }
    gaps.sort_unstable();
    let p99 = if gaps.is_empty() {
        0
    } else {
        gaps[((gaps.len() as f64 * 0.99) as usize).min(gaps.len() - 1)]
    };
...
fn main() {
    ...
    println!(
        "N={N}-name schedule, saturated sources; load = # active names (contention). 'token' = grant.\n"
    );
    let scheds = [
        ("fixed TDMA", Sched::Tdma),
        ("pure CCLF", Sched::Cclf),
        ("demand-adaptive", Sched::Demand),
    ];
    let actives = [2usize, 4, 8, 16];

    println!(
        "  active   scheduler         thru    idle%   coll%   mean-gap   p99-gap   (gap = slots between a name's turns)"
    );
    ...
    println!(
        "takeaway (the token concept, quantified): fixed name-TDMA is deterministic — every name's turn"
    );
    println!(
        "comes exactly every N slots (p99-gap = {N}) — but it WASTES the idle slots of silent names, so"
    );
    println!(
        "with few active names throughput craters (2 active → thru {:.2}). Pure CCLF reuses every slot,",
        2.0 / N as f64
    );
    println!(
        "so it's great with few contenders, but its collision rate climbs with them — at 16 active it"
    );
    println!(
        "COLLAPSES (throughput → the p_col ceiling) and its access gap goes ragged. Demand-adaptive wins"
    );
    println!(
        "BOTH ends: it tracks CCLF's high throughput when names are few, tracks TDMA's collision-free"
    );
    println!(
        "determinism when names are many, and keeps a bounded owner-slot gap throughout. That is the"
    );
    println!(
        "token — a name-keyed, clock-derived, RECLAIMABLE grant — earning its keep with no host identity."
    );
