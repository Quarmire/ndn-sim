...
async fn fetch(engine: &ndn_engine::ForwarderEngine, name: &str) -> Option<(Vec<u8>, Duration)> {
    ...
    match consumer.fetch_with(b).await {
        Ok(d) => Some((
            d.content().map(|c| c.to_vec()).unwrap_or_default(),
            t.elapsed(),
        )),
        Err(_) => None,
    }
...
async fn main() {
    ...
    {
        let mut sim =
            Simulation::new().with_radio_medium(Arc::new(FreeSpacePathLoss::default()), 7);
        ...
        fabric
            .route_over_radio(c, &"/clip".parse::<Name>().unwrap())
            .unwrap();

        let prod = fabric
            .engine_of(p)
            .unwrap()
            .register_producer("/clip", CancellationToken::new());
        let serve = tokio::spawn(async move {
            let _ = prod
                .serve(|i, r| async move {
                    let _ = r
                        .respond((*i.name).clone(), bytes::Bytes::from_static(b"frame-data"))
                        .await;
                })
                .await;
        });
        ...
        let cached =
            tokio::time::timeout(Duration::from_millis(300), fetch(&eng_c, "/clip/seg0")).await;
        println!(
            "2. CS admission:    producer OFF → /clip/seg0 {} — this Data carried no FreshnessPeriod, so",
            match cached {
                Ok(Some(_)) => "still served (cached)",
                _ => "gone",
            }
        );
        println!(
            "                    the DefaultAdmissionPolicy cached nothing (freshness=0 is non-cacheable;"
        );
        println!(
            "                    a producer that stamps freshness, as ndn-sim's AppSpec does, gets CS hits)."
        );
    ...
    // ---- 2. multi-hop over a range-limited medium (the honest test) ------------------------------
    {
        ...
        fabric.engine_of(r).unwrap().strategy_table().insert(
            &clip,
            Arc::new(ndn_strategy::BroadcastStrategy::new())
        ...
        let prod = fabric
            .engine_of(p)
            .unwrap()
            .register_producer("/relayed", CancellationToken::new());
        let serve = tokio::spawn(async move {
            let _ = prod
                .serve(|i, rp| async move {
                    let _ = rp
                        .respond((*i.name).clone(), bytes::Bytes::from_static(b"two-hop"))
                        .await;
                })
                .await;
        });
        ...
        let got = tokio::time::timeout(
            ...
            fetch(&fabric.engine_of(c).unwrap(), "/relayed/x"),
        )
        .await;
        println!(
            ...
            links,
            match got {
                Ok(Some(_)) => "SUCCEEDED — relay re-broadcast worked",
                _ => "TIMED OUT",
            }
        );
        serve.abort();
