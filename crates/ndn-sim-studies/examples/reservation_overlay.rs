...
//!
//! `cargo run --example reservation_overlay --release -p ndn-sim-studies`. Writes CSV + JSON.
...
fn run(arm: Arm, load: f64, frac_lat: f64, frac_foreign: f64, seed: u64) -> Res {
    ...
    let is_foreign: Vec<bool> = (1..=N)
        .map(|_| (xs(&mut rng) as f64 / u64::MAX as f64) < frac_foreign)
        .collect();
    ...
    for f in 0..SUPERFRAMES {
        ...
        if arm == Arm::Overlay {
            ...
            let mut cand: Vec<usize> = (1..=N)
                .filter(|&i| backlog[i].is_some() && !is_lat[i - 1] && !is_foreign[i - 1])
                .collect();
            cand.sort_by_key(|&i| std::cmp::Reverse(f - backlog[i].unwrap())); // oldest packet first
            for i in cand {
                for (s, a) in assigned.iter_mut().enumerate() {
                    if !*a {
                        *a = true;
        ...
        for &(i, t) in &sends {
            let defer = !is_foreign[i - 1]
                ...
                && kept
                    .iter()
                    .any(|&(j, tj)| hears(i, j) && tj <= t && t < tj + air);
            if !defer {
                kept.push((i, t));
            }
        }
...
fn agg(arm: Arm, load: f64, frac_lat: f64, frac_foreign: f64) -> (f64, f64, f64, f64) {
    ...
    let p99 = |v: &mut Vec<u64>| {
        ...
        if v.is_empty() {
            ...
            v[(v.len() * 99 / 100).min(v.len() - 1)] as f64 / 1e3
        }
    };
    (
...
fn main() {
    use std::io::Write;
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/data/reservation-overlay");
    let _ = std::fs::create_dir_all(dir);
    let mut csv = std::fs::File::create(format!("{dir}/overlay.csv")).unwrap();
    writeln!(
        csv,
        "sweep,arm,x,lat_delivery,lat_p99_us,bulk_delivery,bulk_p99_us"
    )
    .unwrap();

    println!(
        "Reservation overlay over the real RadioBus ({N} tx, hidden terminals, tight clock 7µs).\n"
    );
    ...
    println!(
        "{:<12}{:>12}{:>13}{:>13}{:>13}{:>13}",
        "load", "arm", "delivery", "p99 µs", "delivery", "p99 µs"
    );
    for &load in &[0.1f64, 0.3, 0.6, 0.9] {
        for (nm, arm) in [
            ("contention", Arm::Contention),
            ("tdma", Arm::Tdma),
            ("overlay", Arm::Overlay),
        ] {
            let (ld, lp, bd, bp) = agg(arm, load, 0.5, 0.0);
            println!(
                "{:<12}{:>12}{:>12.0}%{:>12.0}{:>12.0}%{:>12.0}",
                format!("{load:.1}"),
                nm,
                ld * 100.0,
                lp,
    ...
    println!(
        "{:<14}{:>14}{:>14}{:>16}{:>14}",
        "foreign %", "lat delivery", "lat p99 µs", "bulk delivery", "bulk p99 µs"
    );
    for &ff in &[0.0f64, 0.1, 0.25, 0.5] {
        let (ld, lp, bd, bp) = agg(Arm::Overlay, 0.6, 0.5, ff);
        println!(
            "{:<14}{:>13.0}%{:>14.0}{:>15.0}%{:>14.0}",
            format!("{:.0}", ff * 100.0),
            ld * 100.0,
            lp,
            bd * 100.0,
    ...
    println!(
        "  class like tdma at high load — dominating both. SWEEP 2: latency degradation vs foreign share"
    );
    println!(
        "  is the honest coexistence cost (self-announcing reservations + CCA bound it, SIC would too)."
    );
