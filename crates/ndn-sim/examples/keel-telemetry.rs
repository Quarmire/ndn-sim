...
fn main() {
    ...
    let stream: Vec<FabricGauges> = DesKernel::new().run(|k: Arc<dyn SimKernel>| async move {
        ...
        for (i, c) in consumers.into_iter().enumerate() {
            fabric
                .route_over_radio(c, &"/svc".parse::<Name>().unwrap())
                .unwrap();
    ...
    println!(
        "{} fabric-gauge samples; final airtime {} µs",
        stream.len(),
        stream
            .last()
    ...
    for m in gui
        .lenses()
        .into_iter()
        .filter(|m| m.intent == "series.window")
    {
        println!(
            "  {}",
            gui.render(m, &stream)
    ...
    println!(
        "  select(≥Approximate) → {:?} (the lossless SVG)",
        gui.select_for("series.window", Floor::Approximate)
            .unwrap()
            .verdict
    );
    ...
    println!(
        "\n[cli] select(≥Approximate) → {}",
        cli.render(ascii, &stream).unwrap().body
    );
    println!(
        "  select(≥Express) → {:?} (CLI filtered out — no lossless offer)",
        cli.select_for("series.window", Floor::Express)
            .map(|m| &m.verdict)
    );
    ...
    println!(
        "\nwrote keel-series-window.html ({} bytes) — open it: the pick, the fallbacks, the losses, on one page.",
        html.len()
    );
    println!(
        "one metric, three surfaces, competing offers, declared losses, a deterministic choice — a human watching it happen."
    );
}
