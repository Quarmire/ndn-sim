//! `Simulation` — multi-node topology builder for in-process NDN networks of
//! forwarders connected by [`SimLink`](crate::SimLink)s.
...
/// Builder for a multi-node simulation topology. Register nodes / links /
/// routes, then [`start`](Self::start) instantiates everything and returns
/// a [`RunningSimulation`].
...
impl Simulation {
    ...
    /// Pre-install a FIB route: packets for `prefix` at `node` forward toward
    /// `nexthop_node` via the SimLink face connecting them.
    ...
    pub async fn start(self) -> Result<RunningSimulation> {
        ...
        // (from_node, to_node) -> FaceId at from_node, for route installation.
