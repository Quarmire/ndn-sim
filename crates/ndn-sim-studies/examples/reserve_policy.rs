...
//!
//! `cargo run --example reserve_policy --release -p ndn-sim-studies`. Writes CSV.
...
fn run(pol: Pol, misleading_occ: bool, seed: u64) -> (f64, f64, f64) {
    ...
    for sf in 0..SUPERFRAMES {
        ...
        for nd in nodes.iter_mut().take(na) {
            if nd.backlog.is_none() {
                nd.backlog = Some(sf);
            }
        }
        ...
        let mut slot_taken = [false; NSLOTS];
