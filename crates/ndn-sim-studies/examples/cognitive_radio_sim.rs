...
//!
//! Run: `cargo run -p ndn-sim-studies --example cognitive_radio_sim`
...
use ndn_sim_studies::cognition::SimCognition;
...
/// Per-round telemetry sample (the observability time-series the dashboard renders); the round is
/// the sample's index.
struct Rec {
    a_deliv: u32,
    b_deliv: u32,
...
fn real_main() {
    ...
    for (ci, (label, interferer, lbt)) in conditions.iter().enumerate() {
        ...
        let (mut a_deliv, mut b_deliv) = (vec![0f64; nr], vec![0f64; nr]); // per-round cumulative delivered
        let (mut a_mcs, mut b_mcs) = (vec![0f64; nr], vec![0f64; nr]); // per-round chosen MCS
        let mut deferrals = vec![0f64; nr]; // per-round cumulative LBT deferrals — LBT's airtime cost
        ...
        for seed in 0..SEEDS {
            ...
            for (r, rec) in recs.iter().enumerate() {
                a_deliv[r] += rec.a_deliv as f64;
                b_deliv[r] += rec.b_deliv as f64;
                a_mcs[r] += rec.a_mcs.max(0) as f64;
                b_mcs[r] += rec.b_mcs.max(0) as f64;
                deferrals[r] += rec.deferrals as f64;
            }
        }
        let s = SEEDS as f64;
        ta /= s;
        ...
        println!(
            "{}. {label}\n   A←B {ta:5.1}/{sent} (PER {:.2})   B←A {tb:5.1}/{sent} (PER {:.2})   LBT deferrals {:.1}",
            ci + 1,
            per(ta),
            per(tb),
            deferrals.last().copied().unwrap_or(0.0) / s,
        );
        if ci > 0 {
            json.push(',');
        }
