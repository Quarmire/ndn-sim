...
//!
//! Run: `cargo run -p ndn-sim-studies --example mobility_study`
...
fn run(n: usize, speed: f64) -> Result {
    ...
    let nodes: Vec<RandomWaypointMobility> = (0..n)
        .map(|i| RandomWaypointMobility {
            radius: REGION_R,
            speed_mps: speed,
            seed: 0x1234_5678 ^ (i as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15),
        })
        .collect();
    ...
    let (mut ndn_ok, mut ip_ok, mut breaks, mut hops_sum, mut hops_cnt, mut total) =
        (0u64, 0u64, 0u64, 0.0f64, 0u64, 0u64);
    for &(c, p) in &pairs {
        ...
        for e in 0..epochs {
            ...
            let intact = committed.as_ref().is_some_and(|r| route_up(r, &adj));
    ...
    Result {
        ...
        ndn_hops: if hops_cnt > 0 {
            hops_sum / hops_cnt as f64
        } else {
            0.0
        },
    }
...
fn main() {
    println!(
        "Mobility study — RandomWaypoint, disc r={REGION_R}m, comm r={COMM_R}m, repair={REPAIR}s\n"
    );
    ...
    for v in vel {
        ...
        a_json.push(format!(
            "{{\"v\":{v},\"ndn\":{:.4},\"ip\":{:.4},\"lam\":{:.4},\"hops\":{:.2}}}",
            r.ndn, r.ip, r.lam, r.ndn_hops
        ));
    }
    println!(
    ...
    println!(
        "    The gap peaks at moderate churn — paths exist but change faster than IP can re-commit."
    );
